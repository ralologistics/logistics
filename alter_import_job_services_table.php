...
echo "<h2>Adding booking_id column to import_job_services table</h2><br>";

// Check if table exists
$table_check = $conn->query("SHOW TABLES LIKE 'import_job_services'");
if ($table_check->num_rows == 0) {
    echo "ERROR: import_job_services table does not exist!<br>";
    echo "Please create the table first using create_import_job_services_table.php<br>";
    $conn->close();
    exit;
}
...
if ($result && $result->num_rows > 0) {
    ...
    $count_result = $conn->query("SELECT COUNT(*) as cnt FROM import_job_services");
    ...
    if ($has_data) {
        ...
        $alter_sql = "ALTER TABLE `import_job_services` 
            ADD COLUMN `booking_id` VARCHAR(255) NULL AFTER `import_job_booking_id`";
...
$check_null = $conn->query("SELECT COUNT(*) as cnt FROM import_job_services WHERE booking_id IS NULL");
...
if ($has_nulls) {
    ...
    $fk_check_sql = "SELECT CONSTRAINT_NAME 
                     ...
                     AND TABLE_NAME = 'import_job_services' 
                     AND CONSTRAINT_NAME = 'fk_import_job_services_booking_id'";
    ...
    if ($fk_result && $fk_result->num_rows > 0) {
        ...
        $fk_sql = "ALTER TABLE `import_job_services` 
            ADD CONSTRAINT `fk_import_job_services_booking_id` 
        ...
        if ($conn->query($fk_sql) === TRUE) {
            ...
            echo "Make sure all booking_id values in import_job_services exist in job_bookings table.<br>";
        }
    }
}

echo "<br><br>Done!<br>";
echo "<a href='check_import_job_services_table.php'>Check table structure</a>";
