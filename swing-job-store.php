...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($customer_id) || empty($from_location) || empty($document_received_at)) {
        header("Location: swing-job-form.php?error=Required fields are missing");
        exit;
    }
    ...
    try {
        // Generate job_no if not provided
        if ($id) {
            // Update existing booking
            $stmt = $conn->prepare("UPDATE swing_job_bookings SET
                customer_id=?,
                from_location=?,
                document_received_at=?
                WHERE id=?");
            $stmt->bind_param("issi", $customer_id, $from_location, $document_received_at, $id);
            if (!$stmt->execute()) {
            ...
            $stmt = $conn->prepare("INSERT INTO swing_job_bookings (
                customer_id,
                job_booking_id,
                from_location,
                document_received_at
            ) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("iiss", $customer_id, $job_booking_id, $from_location, $document_received_at);
            if (!$stmt->execute()) {
            ...
            // Generate job_no using format: SWING-YYYY-N (e.g., SWING-2026-1)
            $current_year = date('Y');
            
            // Get the maximum number for this year to determine next number
            $maxQuery = $conn->prepare("SELECT job_no FROM swing_job_bookings WHERE job_no LIKE ? ORDER BY job_no DESC LIMIT 1");
            $likePattern = 'SWING-' . $current_year . '-%';
            ...
            if ($maxRow && !empty($maxRow['job_no'])) {
                // Extract number from job_no (e.g., SWING-2026-5 -> 5)
            ...
            $job_no = 'SWING-' . $current_year . '-' . $nextNumber;
            
            $stmtUpdate = $conn->prepare("UPDATE swing_job_bookings SET job_no = ? WHERE id = ?");
        ...
        // -----------------------------
        $stmtDel = $conn->prepare("DELETE FROM containers WHERE job_type = 'swing' AND job_id = ?");
        ...
        if (!empty($containers)) {
            ...
            foreach ($containers as $c) {
                ...
                if (empty($reference) || empty($container_no) || empty($iso_code_id) || empty($weight) || empty($available_date) || empty($door_type_id)) {
                    continue;
                }
        ...
        // Delete services by job_type='swing' and container_id matching this booking's containers
        if (!empty($containerIds)) {
            $containerIdsStr = implode(',', array_map('intval', $containerIds));
            $deleteQuery = "DELETE FROM import_job_services WHERE job_type = 'swing' AND container_id IN ($containerIdsStr)";
        ...
        echo "<br><a href='swing-job-form.php" . ($id ? "?id=" . (int)$id : "") . "'>Go Back</a>";
    } finally {
        $conn->close();
    }
}
