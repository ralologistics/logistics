...
if (isset($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT s.*, j.booking_id FROM cart_job_bookings s LEFT JOIN job_bookings j ON s.job_booking_id = j.id WHERE s.id = ?");
    ...
    // Containers
    $stmt = $conn->prepare("SELECT * FROM containers WHERE job_type = 'cart' AND job_id = ? ORDER BY id ASC");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $containers = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    $stmt->close();

    // Services - Get services by job_type='cart' and container_id from containers
    $containerIds = [];
    if (!empty($containers)) {
    ...
    if (!empty($containerIds)) {
        ...
        $servicesQuery = "
            ...
            WHERE ijs.job_type = 'cart' AND ijs.container_id IN ($containerIdsStr)
            ORDER BY ijs.id ASC
        ";
    ...
    // Notes - Get notes by job_type='cart' and booking_id from containers
    if (!empty($containerBookingIds)) {
        ...
        $notesQuery = "
            ...
            WHERE ijn.job_type = 'cart' AND ijn.booking_id IN ($bookingIdsStr)
            ORDER BY ijn.id ASC
        ";
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <div class="container-fluid">
                    <h4 class="page-title">Cart Job Booking</h4>
                </div>
            </section>

            <!-- Main content -->
            <section class="content">
                <form action="cart-job-store.php" method="POST">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group mt-4">
                                        ...
                                        <input type="text" name="to_location" class="form-control" placeholder="To" required
                                            value="<?php echo ($edit && !empty($booking)) ? htmlspecialchars($booking['to_location'] ?? '') : ''; ?>">
                                    </div>
                        ...
                        <div class="col-md-9">
                            <div class="card">
                                ...
                                <div class="card-body">
                                        ...
                                        <tbody>
                                            <?php foreach ($containers as $idx => $c): ?>
                                                <tr>
                                                    ...
                                                    <input type="hidden" name="containers[<?php echo $idx; ?>][job_type]" value="cart">
                                    ...
                                    <div class="text-right mt-4">
                                        <a href="cart-job-list.php" class="btn btn-light">Clear</a>
    ...
    <script>
        (function () {
            ...
            const requiredContainerIds = ['c_booking_id', 'c_reference', 'c_container_no', 'c_weight', 'c_available_date', 'c_door_type_id', 'c_service_type_id'];
            const allContainerIds = [
                'c_booking_id', 'c_reference', 'c_container_no', 'c_iso_code_id', 'c_weight',
                'c_available_date', 'c_xray', 'c_dgs', 'c_ready_now', 'c_door_type_id', 'c_service_type_id'
            ];
            ...
            if (containerAddBtn && containersTbody) {
                containerAddBtn.addEventListener('click', () => {
                    ...
                    row.innerHTML = `
                        ...
                        <input type="hidden" name="containers[${idx}][job_type]" value="cart">
